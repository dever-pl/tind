...
test_that("'.validate_y' works correctly", {
    ...
    names(yyv) <- nms
    yy <- .validate_y(yyv)
    expect_identical(unname(!is.na(yy)), unname(.valid_y0(yyv)))
    expect_identical(unname(yy[!is.na(yy)]), unname(yyv[.valid_y0(yyv)]))
    expect_identical(names(yy), nms)
})
...
test_that("'.is.leap_year' works correctly", {
    ...
    ly <- .is.leap_year(yy)
    expect_identical(unname(ly), ly0)
    expect_identical(.is.leap_year(yy), .is.leap_year0(yy))
    expect_identical(.is.leap_year(NA_integer_), NA)
})
...
test_that("'.y2char' works correctly", {
    ...
    expect_identical(as.integer(yychar), unname(yy))
    expect_identical(.y2char(NA_integer_), NA_character_)
    expect_identical(names(yychar), nms)
})


test_that("'.y2yf' works correctly", {
    yyf <- .y2yf(yy)
    expect_identical(unname(yyf), as.double(yy))
    expect_identical(names(yyf), nms)
    expect_identical(.y2yf(NA_integer_), NA_real_)
})
...
test_that("'.floor_y' and '.ceiling_y' work correctly", {
    ...
    for (n in ns) {
        fy <- .floor_y(yy, n)
        cy <- .ceiling_y(yy, n)
        expect_identical(.floor_y(fy, n), fy)
        expect_identical(.ceiling_y(cy, n), cy)
        expect_true(all(fy %% n == 0L))
        expect_true(all(cy %% n == 0L))
        expect_true(all(fy <= yy & yy <= cy))
        expect_identical(fy == yy, yy == cy)
        if (n == 1L) {
            expect_identical(fy, yy)
            expect_identical(cy, yy)
        } else {
            expect_identical(unname(fy %% n), rep(0L, NN))
            expect_identical(unname(cy %% n), rep(0L, NN))
        }
    }
})
...
test_that("'.validate_q' and '.validate_yq' work correctly", {
    ...
    qqv[(NN - 3L):NN] <- 0L:3L
    qq <- .validate_q(qqv)
    expect_identical(!is.na(qq), .valid_q0(qqv))
    expect_identical(qq[!is.na(qq)], qqv[.valid_q0(qqv)])
    expect_identical(.validate_yq(qqv %/% 4L, qqv %% 4L + 1L), qq)
})
...
test_that("'.q2char' works correctly", {
    ...
    expect_identical(as.integer(sub("Q[0-9]$", "", qqchar)), as.integer(yy))
    expect_identical(as.integer(sub("^[0-9]{4}Q", "", qqchar)), as.integer(q))
    expect_identical(.q2char(NA_integer_), NA_character_)
    expect_identical(names(qqchar), nms)
})


test_that("'.q2qrtr' works correctly", {
    expect_identical(unname(.q2qrtr(qq)), q)
})


test_that("'.q2yf' works correctly", {
    expect_equal(.q2yf(qq), yy + (q - 1) / 4)
    expect_identical(.q2yf(NA_integer_), NA_real_)
})


test_that("'.q2y' works correctly", {
    expect_identical(.q2y(qq), yy)
})


test_that("'.y2q' works correctly", {
    expect_identical(unname(.q2char(.y2q(yy))), paste0(.y2char(yy), "Q1"))
})
...
test_that("'.floor_q' and '.ceiling_q' work correctly", {
    ...
    for (n in ns) {
        ...
        expect_identical(.floor_q(fq, n), fq)
        expect_identical(.ceiling_q(cq, n), cq)
        expect_true(all(fq <= qq & qq <= cq))
        expect_identical(fq == qq, qq == cq)
        if (n == 1L) {
            expect_identical(fq, qq)
            expect_identical(cq, qq)
        } else {
            expect_identical(unname(fq %% 4L) %% n, rep(0L, NN))
            expect_identical(unname(cq %% 4L) %% n, rep(0L, NN))
        }
    }
})
...
test_that("'.validate_m' and '.validate_ym' work correctly", {
    ...
    mm <- .validate_m(mmv)
    expect_identical(!is.na(mm), .valid_m0(mmv))
    expect_identical(mm[!is.na(mm)], mmv[.valid_m0(mmv)])
    expect_identical(.validate_ym(mmv %/% 12L, mmv %% 12L + 1L), mm)
})
...
test_that("'.m2char' works correctly", {
    ...
    expect_identical(as.integer(sub("-[0-9]{2}$", "", mmchar)), as.integer(yy))
    expect_identical(as.integer(sub("^[0-9]{4}-", "", mmchar)), as.integer(m))
    expect_identical(.m2char(NA_integer_), NA_character_)
    expect_identical(names(mmchar), nms)
})


test_that("'.m2mnth' works correctly", {
    expect_identical(unname(.m2mnth(mm)), m)
})


test_that("'.m2yf' works correctly", {
    expect_equal(.m2yf(mm), yy + (m - 1) / 12)
    expect_identical(.m2yf(NA_integer_), NA_real_)
})


test_that("'.m2y' works correctly", {
    expect_identical(.m2y(mm), yy)
})


test_that("'.y2m' works correctly", {
    expect_identical(unname(.m2char(.y2m(yy))), paste0(.y2char(yy), "-01"))
})


test_that("'.q2m' works correctly", {
    expect_identical(.m2y(.q2m(qq)), yy)
    expect_identical(unname(.q2m(qq) %% 12L + 1L), (q - 1L) * 3L + 1L)
})


test_that("'.m2q' works correctly", {
    expect_identical(.q2y(.m2q(mm)), yy)
    expect_identical(.m2q(mm) %% 4L + 1L, (mm %% 12L) %/% 3L + 1L)
})
...
test_that("'.floor_m' and '.ceiling_m' work correctly", {
    ...
    for (n in ns) {
        ...
        expect_identical(.floor_m(fm, n), fm)
        expect_identical(.ceiling_m(cm, n), cm)
        expect_true(all(fm <= mm & mm <= cm))
        expect_identical(fm == mm, mm == cm)
        if (n == 1L) {
            expect_identical(fm, mm)
            expect_identical(cm, mm)
        } else {
            expect_identical(unname(fm %% 12L) %% n, rep(0L, NN))
            expect_identical(unname(cm %% 12L) %% n, rep(0L, NN))
        }
    }
})
...
test_that("'.weeks_in_year' works correctly", {
    ...
    expect_identical(unname(winy), winy0)
})
...
test_that("'.validate_w' and '.validate_yw' work correctly", {
    ...
    expect_identical(is.na(ww), !.valid_y0(y0) | (w0 < 1) | (w0 > .weeks_in_year(y0)))
    expect_false(any(is.na(.validate_yw(yy, w))))
    ww <- c(.validate_yw(0, 1) + (-1:0), .validate_yw(9999, 52) + (0:1))
    expect_identical(.validate_w(ww), c(NA_integer_, ww[2L:3L], NA_integer_))
})
...
test_that("'.w2char' works correctly", {
    ...
    expect_identical(as.integer(sub("^[0-9]{4}-W", "", wwchar)), unname(w))
    expect_identical(as.integer(sub("-W[0-9]{2}$", "", wwchar)), unname(yy))
    expect_identical(.w2char(NA_integer_), NA_character_)
    expect_identical(names(wwchar), nms)
})


test_that("'.w2week' works correctly", {
    expect_identical(unname(.w2week(ww)), w)
})


test_that("'.w2y' works correctly", {
    expect_identical(.w2y(ww), yy)
})


test_that("'.y2w' works correctly", {
    expect_identical(.y2w(yy), .validate_yw(yy, 1))
})
...
test_that("'.floor_w' and '.ceiling_w' work correctly", {
    ...
    for (n in ns) {
        ...
        expect_identical(fw == ww, ww == cw)
        expect_identical(.floor_w(fw, n), fw)
        expect_identical(.ceiling_w(cw, n), cw)
        if (n == 1L) {
            expect_identical(fw, ww)
            expect_identical(cw, ww)
        } else {
            expect_identical(unname(.w2week(fw) - 1L) %% n, rep(0L, NN))
            expect_identical(unname(.w2week(cw) - 1L) %% n, rep(0L, NN))
        }
    }
})
...
test_that("'.inc_w_by_y' works correctly", {
    ...
    expect_identical(unname(y1 - y0), dy)
    i53 <- (w0 == 53)
    expect_identical(w1[!i53], w0[!i53])
    expect_identical(w1[i53], unname(.w2week(.y2w(y0[i53] + dy[i53] + 1L) - 1L)))
})
...
test_that("'.days_in_year' works correctly", {
    ...
    expect_identical(unname(dy), dy0)
})
...
test_that("'.days_in_quarter' works correctly", {
    ...
    expect_identical(unname(dq), dq0)
})
...
test_that("'.days_in_month' works correctly", {
    ...
    expect_identical(unname(dm), dm0)
})
...
test_that("'.validate_ymd' works correctly", {
    ...
    expect_identical(unname(ymd), as.integer(DD))
})
...
test_that("'.validate_d' works correctly", {
    ...
    vd <- .validate_d(vd)
    expect_identical(vd[3L:(NN + 2L)], ymd)
...
test_that("'.d2char' works correctly", {
    ...
    expect_identical(as.integer(gsub("^([0-9]{4})-([0-9]{2})-([0-9]{2})$", "\\1", ddchar)),
                     as.integer(yy))
    expect_identical(as.integer(gsub("^([0-9]{4})-([0-9]{2})-([0-9]{2})$", "\\2", ddchar)),
                     as.integer(m))
    expect_identical(as.integer(gsub("^([0-9]{4})-([0-9]{2})-([0-9]{2})$", "\\3", ddchar)),
                     as.integer(d))
    expect_identical(.d2char(NA_integer_), NA_character_)
})


test_that("'.day_of_year' works correctly", {
    DD <- as.Date(.d2char(dd))
    expect_identical(unname(.day_of_year(dd)), as.integer(format(DD, "%j")))
})


test_that("'.day_of_week' works correctly", {
    DD <- as.Date(.d2char(dd))
    expect_identical(unname(.day_of_week(dd)), as.integer(format(DD, "%u")))
})


test_that("'.d2day' works correctly", {
    expect_identical(unname(.d2day(dd)), d)
})


test_that("'.d2y' works correctly", {
    expect_identical(.d2y(dd), yy)
})


test_that("'.d2q' works correctly", {
    expect_identical(.d2q(dd), .m2q(mm))
})


test_that("'.d2m' works correctly", {
    expect_identical(.d2m(dd), mm)
})


test_that("'.m2d' works correctly", {
    dd <- .m2d(mm)
    expect_identical(dd, .validate_ymd(yy, m, 1))
})


test_that("'.last_day_in_month' works correctly", {
    dd <- .last_day_in_month(mm)
    expect_identical(.m2d(mm) - 1L + .days_in_month(mm), dd)
})


test_that("'.y2d' works correctly", {
    dd <- .y2d(yy)
    expect_identical(dd, .validate_ymd(yy, 1, 1))
})


test_that("'.q2d' works correctly", {
    dd <- .q2d(qq)
    expect_identical(dd, .validate_ymd(yy, (q - 1) * 3 + 1, 1))
})
...
test_that("'.w2d' works correctly", {
    ...
    expect_identical(names(dd), nms)
    expect_identical(unname(ww),
                     .validate_yw(as.integer(format(as.Date(wD), "%G")),
                                  as.integer(format(as.Date(wD), "%V"))))
    expect_identical(as.integer(format(wD, "%u")), rep(1L, NN))
})
...
test_that("'.d2jdn' and '.jdn2d' work correctly", {
    expect_identical(.d2jdn(.validate_ymd(2000, 1, 1)), 2451545L)
    DD <- as.Date(.d2char(dd))
    expect_identical(unname(.d2jdn(dd)), 2451545L + as.integer(DD - as.Date("2000-01-01")))
    expect_identical(.jdn2d(.d2jdn(dd)), dd)
})
...
test_that("'.easter' works correctly", {
    ...
    expect_identical(.easter(.d2y(es)), es)
})
...
test_that("'.floor_d' and '.ceiling_d' work correctly", {
    ...
    for (n in ns) {
        fd <- .floor_d(dd, n)
        cd <- .ceiling_d(dd, n)
        expect_identical(.floor_d(fd, n), fd)
        expect_identical(.ceiling_d(cd, n), cd)
        expect_true(all(fd <= dd & dd <= cd))
        expect_identical(fd == dd, dd == cd)
        if (n == 1L) {
            expect_identical(fd, dd)
            expect_identical(cd, dd)
        } else if (n == 2) {
...
test_that("'.inc_d_by_m' works correctly", {
    ...
    m1 <- .d2m(dd1)
    d1 <- unname(.d2day(dd1))
    expect_identical(unname(m1 - m0), dm)
    i28l <- (d0 <= 28L)
    expect_identical(d1[i28l], d0[i28l])
    expect_identical(d1[!i28l], unname(pmin(d0[!i28l], .days_in_month(m1[!i28l]))))
})
...
test_that("'.validate_yj' works correctly", {
    ...
    expect_identical(as.integer(format(nthD, "%Y")), unname(yy))
    expect_identical(as.integer(format(nthD, "%j")), unname(nn))
    expect_identical(.validate_yj(yy, .day_of_year(dd)), dd)
    expect_identical(.validate_yj(c(    2000L,     2016L), 366L),
                     .validate_ymd(c(2000, 2016), 12, 31))
    expect_identical(.validate_yj(c(2020L, 2020L), c(NA, 366L)),
                     .validate_ymd(c(NA, 2020), 12, 31))
    expect_identical(.validate_yj(yy, integer()), integer())
    expect_identical(.validate_yj(integer(), nn), integer())
...
test_that("'.last_dw_in_month' works correctly", {
    ...
    expect_identical(unname(.day_of_week(dd)), dw)
    expect_true(all(.d2m(dd) == mm))
    expect_false(any(.d2m(dd + 7L) == mm))
    expect_identical(.last_dw_in_month(integer(), mm), integer())
    expect_identical(.last_dw_in_month(dw, integer()), integer())
    warn <- "longer object length is not a multiple of shorter object length"
    ...
    expect_identical(unname(dd), rep(NA_integer_, NN))
})
...
test_that("'.validate_ywu' works correctly", {
    ...
    expect_identical(.d2w(dd), ww)
    expect_identical(as.integer(format(ddD, "%u")), dw)
    expect_identical(.validate_ywu(yy, w, integer()), integer())
    expect_identical(.validate_ywu(integer(), integer(), dw), integer())
    warn <- "longer object length is not a multiple of shorter object length"
    ...
    expect_identical(unname(.validate_ywu(yy[1L:8L], w[1L:8L], c(dw[1L:7L], 8L))),
...
test_that("'.nth_dw_in_month' works correctly", {
    ...
    expect_identical(.nth_dw_in_month(integer(), dw, mm), integer())
    expect_identical(.nth_dw_in_month(nn, integer(), mm), integer())
    expect_identical(.nth_dw_in_month(nn, dw, integer()), integer())
    warn <- "longer object length is not a multiple of shorter object length"
    ...
    expect_identical(unname(.nth_dw_in_month(6L, dw, mm)), rep(NA_integer_, NN))
    expect_identical(unname(.nth_dw_in_month(nn, 8L, mm)), rep(NA_integer_, NN))
})
