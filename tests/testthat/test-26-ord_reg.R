...
test_that("'is.ordered_t' method works correctly", {
    ...
    expect_identical(is.ordered_t(dd), .isorderedt0(dd))
    expect_identical(is.ordered_t(tt), .isorderedt0(tt))
    dd <- sort(dd)
    tt <- as.date_time(dd, "UTC")
    expect_identical(is.ordered_t(dd), .isorderedt0(dd))
    expect_identical(is.ordered_t(tt), .isorderedt0(tt))
    dd <- unique(dd)
    tt <- as.date_time(dd, "UTC")
    expect_identical(is.ordered_t(dd), TRUE)
    expect_identical(is.ordered_t(tt), TRUE)
    expect_identical(is.ordered_t(as.POSIXct(tt)), TRUE)
    dd[sample.int(length(dd), 1L)] <- NA
    tt <- as.date_time(dd, "UTC")
    expect_identical(is.ordered_t(dd), FALSE)
    expect_identical(is.ordered_t(tt), FALSE)
    expect_identical(is.ordered_t(as.Date(dd)), FALSE)
    expect_identical(is.ordered_t(as.POSIXct(dd)), FALSE)
...
test_that("'is.regular' and 'as.regular' work correctly", {
    ...
    for (tp in c("d", "w", "m", "q", "y")) {
        ...
        expect_identical(as.regular(x0), x0)
        ...
        expect_identical(as.regular(x11), x11)
        x2 <- unique(x1)
        expect_true(is.regular(x2))
        expect_true(is.regular(as.Date(x2)))
        expect_identical(as.regular(x2), x2)
        x3 <- x2[-sample(2L:(length(x2) - 1L), 1L)]
        expect_false(is.regular(x3))
        expect_false(is.regular(as.Date(x3)))
        x4 <- as.regular(x3)
        expect_identical(x4, x2)
        expect_identical(as.regular(x4), x4)
        expect_identical(as.regular(as.Date(x3)), as.date(x4))
        x5 <- x2[-sample(2L:(length(x2) - 1L), 1L)]
        expect_false(is.regular(x5))
        expect_false(is.regular(as.Date(x5)))
        x6 <- as.regular(x5)
        expect_identical(x6, x2)
        expect_identical(as.regular(x6), x6)
        for (hftp in setdiff(.hi_res_cast(tp), "t")) {
            x0a <- as.tind(x0, hftp)
            expect_true(is.regular(x0a))
            expect_identical(as.regular(x0a), x0a)
            x1a <- as.tind(x1, hftp)
            expect_false(is.regular(x1a))
            x2a <- as.tind(x2, hftp)
            expect_true(is.regular(x2a))
            expect_identical(as.regular(x2a), x2a)
            x3a <- as.tind(x3, hftp)
            expect_false(is.regular(x3a))
            x4a <- as.regular(x3a)
            expect_identical(x4a, x2a)
            expect_identical(as.regular(x4a), x4a)
            x5a <- as.tind(x5, hftp)
            x6a <- as.regular(x5a)
            expect_identical(x6a, x2a)
            expect_identical(as.regular(x6a), x6a)
        }
        tz <- sample(tzs, 1L)
