...
test_that("'floor_t', 'ceiling_t', 'round_t', and 'trunc_t' work correctly", {
    ...
    for (tp in tu) {
        ...
        for (un in c(.lo_res_cast(tp), tp)) {
            ...
            for (ns in nss) {
                ...
                expect_identical(fx, floor_t(xx, paste0(ns, un)))
                expect_identical(fx, floor_t(xx, as.tdiff(paste0(ns, un))))
                expect_identical(cx, ceiling_t(xx, paste0(ns, un)))
                expect_identical(cx, ceiling_t(xx, as.tdiff(paste0(ns, un))))
                expect_identical(cxf, ceiling_t(xx, paste0(ns, un), ceiling = "following"))
                expect_identical(cxl, ceiling_t(xx, paste0(ns, un), ceiling = "last"))
                expect_identical(rx, round_t(xx, paste0(ns, un)))
                expect_identical(rx, round_t(xx, as.tdiff(paste0(ns, un))))
                expect_identical(rxf, round_t(xx, paste0(ns, un), ceiling = "following"))
                expect_identical(rxl, round_t(xx, paste0(ns, un), ceiling = "last"))
                if (ns == 1L) {
                    tx <- trunc_t(xx, un)
                    if (un == tp) {
    ...
    xx <- get(paste0(tp, tp))
    un <- sample(c(.lo_res_cast(tp), tp), 1L)
    expect_identical(trunc_t(xx[0L], un), tind(type = un))
    expect_identical(trunc_t(xx[0L], tp), xx[0L])
    expect_identical(floor_t(xx[0L], un), xx[0L])
    expect_identical(ceiling_t(xx[0L], un), xx[0L])
    expect_identical(round_t(xx[0L], un), xx[0L])

    # t, h
    xx <- tt
    xxh <- as.tind(xx, type = "h")
    un <- sample(c("d", "w", "m", "q", "y"), 1L)
    expect_identical(trunc_t(xx, un), as.tind(xx, type = un))
    expect_identical(trunc_t(xx[0L], un), as.tind(xx[0L], type = un))
    expect_identical(floor_t(xx[0L], un), xx[0L])
    expect_identical(ceiling_t(xx[0L], un), xx[0L])
    expect_identical(round_t(xx[0L], un), xx[0L])
    err <- "^invalid time unit"
    ...
    for (un in c("s", "min", "h")) {
        ...
        for (ns in nss) {
            ...
            if (ns == 1L) {
                tx <- trunc_t(xx, un)
                expect_identical(tx, fx)
                txh <- trunc_t(xxh, un)
                expect_identical(txh, fxh)
                expect_identical(trunc_t(xx[0L], un), xx[0L])
                expect_identical(trunc_t(xxh[0L], un), xxh[0L])
            }
            ina <- is.na(xx)
            expect_true(identical(ina, is.na(fx)) &&
                        identical(ina, is.na(cx)) &&
    ...
    expect_identical(floor_t(xx, 2), as.tind((as.integer(xx) %/% 2L) * 2L, type = "i"))
    # n
