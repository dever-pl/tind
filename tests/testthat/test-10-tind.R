...
test_that("'tind' (constructor) works correctly", {
    ...
    expect_identical(.get.tz(x), NULL)
    attributes(x) <- NULL
    expect_identical(x, y)
    expect_identical(tind(y = integer()), tind(type = "y"))
    ...
    x <- tind(y = y, m = m, d = d)
    expect_identical(x, tind(m = m, d = d, y = y))
    expect_identical(x, tind(d = d, m = m, y = y))
    expect_identical(.get.type(x), "d")
    expect_identical(.get.tz(x), NULL)
    attributes(x) <- NULL
    expect_identical(x, .validate_ymd(y, m, d))
    ...
    x <- tind(y = y, w = w, u = u)
    expect_identical(x, tind(w = w, u = u, y = y))
    expect_identical(x, tind(u = u, w = w, y = y))
    expect_identical(.get.type(x), "d")
    expect_identical(.get.tz(x), NULL)
    attributes(x) <- NULL
    expect_identical(x, .w2d(.validate_yw(y, w)) + u - 1L)
