...
test_that("'.eval_clndr' works correctly", {
    expect_identical(.eval_clndr(dd, .calendar_PL, bd.only = TRUE),
                     as.vector(.calendar_PL1(dd)))
    expect_identical(.eval_clndr(dd, .calendar_PL1, bd.only = TRUE),
                     as.vector(.calendar_PL1(dd)))
    expect_identical(.eval_clndr(dd, .calendar_PL2, bd.only = TRUE),
                     as.vector(.calendar_PL1(dd)))
    ...
    expect_identical(res0[[1L]], res1[[1L]])
    expect_identical(res0[[1L]], res2[[1L]])
    expect_identical(res0[[2L]], res2[[2L]])
    expect_identical(res1[[3L]], rep(FALSE, NN))
    expect_identical(res2[[3L]], rep(FALSE, NN))
    expect_identical(res1[[2L]], rep(FALSE, NN))
...
test_that("'eval_calendar' works correctly", {
    ...
    expect_identical(names(res0), c("bizdays", "holidays", "otherobs"))
    expect_identical(names(res1), c("bizdays", "holidays", "otherobs"))
    expect_identical(names(res2), c("bizdays", "holidays", "otherobs"))
    expect_true(all(sapply(res0, is.tind)))
    expect_true(all(sapply(res0, function(r) all(r %in% dd))))
    expect_identical(res0[[1L]], res1[[1L]])
    expect_identical(res0[[1L]], res2[[1L]])
    expect_identical(res0[[2L]], res2[[2L]])
    expect_identical(res1[[2L]], tind(type = "d"))
    expect_identical(res1[[3L]], tind(type = "d"))
    expect_identical(res2[[3L]], tind(type = "d"))
    res0. <- .eval_clndr(dd, .calendar_PL)
    expect_equal(res0[[1L]], dd[res0.[[1L]]])
    expect_true(all(res0[[2L]] == dd[res0.[[2L]]]))
    expect_true(all(res0[[3L]] == dd[res0.[[3L]]]))
    expect_identical(names(res0[[2L]]), names(res0.[[2L]])[res0.[[2L]]])
    expect_identical(names(res0[[3L]]), names(res0.[[3L]])[res0.[[3L]]])
...
test_that("'calendar' works correctly", {
    ...
    id <- grepl(patd, cap)
    expect_identical(which(iy), 1L)
    expect_identical(which(iw), which(im) + 1L)
    ...
    iw <- grepl(patw, cap)
    id <- grepl(patd, cap)
    expect_identical(which(iym)[1L], 1L)
    expect_identical(which(iw), which(iym) + 1L)
    expect_identical(length(which(iym)), 1L)
    ...
    expect_identical(which(iym)[1L], 1L)
    expect_identical(which(iw), which(iym) + 1L)
    expect_identical(length(which(iym)), 2L)
    expect_true(all((iym + iw + id) == 1L))
    # 3
