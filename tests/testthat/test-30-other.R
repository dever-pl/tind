...
test_that("zoo::yearmon and zoo::yearqtr are handled correctly", {
    ...
    expect_identical(ti_type(x, long = FALSE), "m")
    xt <- as.tind(x)
    ...
    expect_true(.tind_coercible(x))
    expect_identical(ti_type(x, long = FALSE), "q")
    xt <- as.tind(x)
    expect_identical(resolution_t(x), resolution_t(xt))
    expect_identical(tspan(x), tspan(xt))
...
test_that("timeDate::timeDate is handled correctly", {
    ...
    for (tz in tzs) {
        ...
        expect_identical(x, timeDate::as.timeDate(xt))
        expect_true(.tind_coercible(x))
        expect_identical(ti_type(x, long = FALSE), "t")
        expect_identical(x@FinCenter, tz)
        expect_identical(as.tind(x), xt)
        expect_identical(tzone(x), tzone(xt))
    }
    if (length(tzs) >= 2L) {
        tsz <- sample(tzs, 2L)
...
test_that("chron::chron, chron::dates, and chron::times are handled correctly", {
    ...
    expect_identical(ti_type(chrt, long = FALSE), "t")
    expect_identical(tzone(chrt), "UTC")
    chrd <- chron::as.dates(td)
    expect_true(.tind_coercible(chrd))
    expect_identical(chron::as.chron(td), chrd)
    expect_identical(ti_type(chrd, long = FALSE), "d")
    chrh <- chron::as.times(th)
    expect_true(.tind_coercible(chrh))
    expect_identical(chron::as.chron(th), chrh)
    expect_identical(ti_type(chrh, long = FALSE), "h")
    expect_identical(as.character(chrt), format(tt, "(%m/%d/%y %T)"))
    expect_identical(as.character(chrd), format(td, "%m/%d/%y"))
    expect_identical(as.character(chrh), format(th, "%T"))
    tt2 <- as.tind(chrt)
    expect_identical(tzone(tt2), "UTC")
    expect_identical(tt2, tt)
    expect_identical(as.tind(chrd), td)
    expect_identical(as.tind(chrh), th)
    expect_identical(ti_type(chrt, long = FALSE), "t")
})
...
test_that("data.table::IDate and data.table::ITime are handled correctly", {
    ...
    expect_identical(ti_type(ID, long = FALSE), "d")
    expect_identical(as.tind(ID), td)
    expect_identical(as.Date(ID), as.Date(td))
    IT <- data.table::as.ITime(th)
    expect_true(.tind_coercible(IT))
    expect_identical(ti_type(IT, long = FALSE), "h")
...
test_that("'date2num' and 'num2date' work correctly", {
    # MATLAB
    dd <- 730377
    expect_identical(num2date(dd, "MATLAB"), as.tind("1999-09-14"))
    expect_identical(date2num(as.tind("1999-09-14"), "MATLAB"), as.integer(dd))
    # Excel
    dd <- 36526
    expect_identical(num2date(dd, "Excel"), as.tind("2000-01-01"))
    expect_identical(date2num(as.tind("2000-01-01"), "Excel"), as.integer(dd))
    # SAS
    dd <- 19068
    expect_identical(num2date(dd, "SAS"), as.tind("2012-03-16"))
    expect_identical(date2num(as.tind("2012-03-16"), "SAS"), as.integer(dd))
    # R
    dd <- sample.int(30000L, 1L)
    expect_identical(num2date(dd, "R"), as.tind(as.Date(dd, origin = "1970-01-01")))
    expect_identical(date2num(as.tind(as.Date(dd, origin = "1970-01-01")), "R"), dd)
    # JDN
    dd <- as.tind(as.Date(dd, origin = "1970-01-01"))
    expect_identical(date2num(dd, "JDN"), jdn(dd))
    expect_identical(num2date(date2num(dd, "JDN"), "JDN"), dd)
    # errors
